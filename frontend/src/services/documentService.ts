            ...
            if (format === 'docx') {
                ...
                if (window.electron) {
                    ...
                    const result = await window.electron.saveFile({
                        fileName: `contrato_${contract.identifier || new Date().getTime()}.docx`,
                    ...
                    if (result.success) {
                        ...
                        toast.error(`Erro ao salvar documento: ${result.error}`);
                    }
                } else {
                    // Versão web
                ...
                if (window.electron) {
                    ...
                    const result = await window.electron.downloadPDF({
                        fileName: `contrato_${contract.identifier || new Date().getTime()}.pdf`,
                        content: Buffer.from(pdfBytes)
                    });

                    if (result.success) {
                        toast.success('Documento PDF salvo com sucesso!');
                    } else {
                        toast.error(`Erro ao salvar PDF: ${result.error}`);
                    }
                } else {
                    // Versão web
