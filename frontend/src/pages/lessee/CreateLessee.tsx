...
const LesseeRegistration = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (id) {
        try {
          const response = await getLesseeById(id);
          if (response) {
            console.log("resposta", response);
            setLesseeData(response);
          } else {
            toast.error("Locatário não encontrado");
          }
        } catch (e) {
          toast.error("Erro ao buscar Locatário");
        }
      } else {
        console.log("Criando novo Locatário");
      }
    };
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      toast.error("Erro ao cadastrar locatário. Tente novamente.");
    } finally {
      setSubmitting(false);
    }
  };
        ...
        >
          {({ isSubmitting, setFieldValue }) => (
            <Form className="space-y-4">
              ...
              >
                {isSubmitting ? "Salvando..." : "Salvar Locatário"}
              </button>
            </Form>
          )}
        </Formik>
...
export default LesseeRegistration;
